...
export class ReservationsService {
    ...
    async createWebhookReservation(guestId : string, guestName : string, checkInTime : string, 
        guestGroupMemberId : string, propertyId : string, roomTypeId : string,
        anticipatedCheckOutTime : string
    ): Promise<any> {
        const utcCheckInDate = this.utilityService.convertToUTCISODateString(checkInTime);
        const utcAnticipatedCheckOutTime = this.utilityService.convertToUTCISODateString(new Date(anticipatedCheckOutTime).toString());
